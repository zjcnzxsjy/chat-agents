...
export function DocumentsCard({
  ...
  const filteredDocuments = useMemo(
    () =>
      documents.filter(
        (doc) => doc.metadata.collectionId === selectedCollection?.id,
      ),
  ...
  const handleUploadStagedFiles = async () => {
    ...
    await handleDocumentFileUpload(fileList, selectedCollection.id);
  ...
  const handleTextUpload = async () => {
    ...
    if (textInput.trim()) {
      ...
      await handleDocumentTextUpload(textInput, selectedCollection.id);
      setTextInput("");
      setIsUploading(false);
      toast.dismiss(loadingToast);
  ...
  const handleChatWithDocuments = async (agent: Agent) => {
    ...
    const chatQueryParams = new URLSearchParams({
      ...
      chatWithCollectionId: selectedCollection.id,
    })?.toString();
