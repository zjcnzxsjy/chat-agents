...
async function uploadDocuments(
  ...
  const url = `${getApiUrlOrThrow().href}api/collection/documents`;
  ...
  if (metadatas) {
    ...
    formData.append("documents", metadatasJsonString);
  }
...
export function useRag(): UseRagReturn {
  ...
  const listDocuments = useCallback(
    async (
      ...
      const url = getApiUrlOrThrow();
      url.pathname = `api/collection/documents`;
  ...
  const deleteDocument = useCallback(
    async (id: string) => {
      ...
      const url = getApiUrlOrThrow();
      url.pathname = `api/collection/documents`;
      url.searchParams.set("fileIds", id);
      ...
      setDocuments((prevDocs) =>
        prevDocs.filter((doc) => doc.metadata.fileId !== id),
      );
    },
  ...
  const createCollection = useCallback(
    async (
      ...
      const res = await response.json();
      setCollections((prevCollections) => [...prevCollections, res.data]);
      return res.data;
    },
    [collections, session],
  );
  ...
  const deleteCollection = useCallback(
    async (collectionId: string): Promise<string | undefined> => {
      ...
      const url = getApiUrlOrThrow();
      url.pathname = `api/collections`;
      url.searchParams.set("ids", collectionId);
